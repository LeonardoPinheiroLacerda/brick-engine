...
export default function dimensions(
    ...
    root.style.setProperty(
        '--sm-button-size-mobile-font-size',
        `${width * 0.04}px`,
    );
    root.style.setProperty(
        '--sm-button-size-mobile-line-height',
        `${width * 0.04}px`,
    );

    root.style.setProperty(
        '--button-size-mobile-font-size',
        `${width * 0.05}px`,
    );
    root.style.setProperty(
        '--lg-button-size-mobile-font-size',
        `${width * 0.055}px`,
    );

    root.style.setProperty(
        '--button-size-mobile-spacing',
        `${width * 0.018}px`,
    );

    root.style.setProperty(
        '--button-border',
        `${width * 0.0045}px solid black`,
    );
