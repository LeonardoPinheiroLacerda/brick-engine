...
Executes all callbacks associated with the given key and event type. Events are only dispatched if the game state is ON (`isOn()`), with the exception of the `POWER` key, which is always processed. 
