...
Creates or updates the session. Early returns if the session is disabled (via `setSessionEnabled`) or if the `SessionModal` (if existing) has not been confirmed/rejected yet.
