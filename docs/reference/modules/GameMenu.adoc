...
Returns `game-menu`. This ensures the menu state is intentionally ignored by the `xref:GameSession.adoc[GameSession]` mechanism, averting logic conflicts that would try to parse the "menu" as a playable state.
