...
Increases the score by `amount * multiplier`. Automatically compares the new score against the persisted `highScore` in `xref:GameState.adoc[GameState]` and updates it if a new record is reached.
...
|===
|Name |Type |Description
|`state` |`xref:../interfaces/modules/State.adoc[State]` |The `xref:GameState.adoc[GameState]` module.
|===

**Behavior**

Implements the `xref:../interfaces/StateSyncable.adoc[StateSyncable]` interface. This connection is used to fetch and update the global high score.
