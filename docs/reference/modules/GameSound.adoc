...
|`sound` |`Sound` |The enum identifier of the sound to play.
|===
...
|===
|Name |Type |Description
|`sound` |`Sound` |The enum identifier of the sound to stop.
|===
...
Inverts the `muted` property in `GameState`. This change is automatically persisted to `LocalStorage` and reflected in the audio output.
...
|===
|Name |Type |Description
|`state` |`State` |The `GameState` instance to link with.
|===

**Behavior**

Implements the `StateSyncable` interface. Subscribes to changes in the `MUTED` property of the provided state and updates the master gain node accordingly.
