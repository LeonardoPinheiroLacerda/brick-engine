...
|===
|Name |Type |Description
|`handler` |`Function` |The callback function receiving the new game instance.
|===

**Behavior**

This is typically set by the engine's main loop (e.g., in `main.ts`) to orchestrate transitions between different game states or modules.

'''

=== `propagateSwitchHandler(game: Game): void`

Propagates the switch handler from an existing game instance to a new one.
...
|===
|Name |Type |Description
|`game` |`Game` |The source game instance that already has the switch handler registered.
|===

**Behavior**

Ensures that the new game instance inherits the same orchestration logic for future switches, maintaining the execution chain.
...
*   **EXIT**: Hard stop of the draw loop. In server mode, the orchestrator typically uses the switch handler to return to the `GameMenu`.
