...
|`modules` |`GameModules` |Access to all engine modules (grid, control, state, etc.).
|===
...
Called when the game state is `gameOver`. Executed after the final `render` call to overlay messages or recovery menus.
...
*   **START_PAUSE**: Starts the game or toggles pause.
