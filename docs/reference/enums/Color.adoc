...
Colors are used throughout the engine to theme the `GameGrid` and the `HUD`. Use these constants when calling methods like `grid.setCellColor(coord, Color.RED)`.
