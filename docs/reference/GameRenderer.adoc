...
`RendererMetrics` - An object containing geometric data for `display`, `hud`, and individual `cells`.

**Behavior**

Calculates metrics once during initialization based on the current canvas size and configuration. These values are used by all other modules (like `GameText`) to ensure pixel-perfect alignment.
...
|`renderer` |`Renderer` |An implementation of the `Renderer` interface (e.g., `DisplayRenderer`).
|===
...
|===
|Name |Type |Description
|`grid` |`Cell[][]` |The current 2D matrix of cells to visualize.
|`modules` |`GameModules` |Reference to all available modules for context-aware rendering.
|===
