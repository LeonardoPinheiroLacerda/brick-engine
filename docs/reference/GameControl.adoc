...
*   **Params**:
    *   `key`: The key to listen for (e.g., `ControlKey.ACTION`).
    *   `type`: The event type (e.g., `ControlEventType.PRESSED`).
    *   `callback`: Function to execute when event triggers.
*   **Returns**:
    *   `void`
*   **Behavior**:
    *   Adds the callback to the internal list for the specified key and type.
...
*   **Params**:
    ...
    *   `type`: The event type.
    *   `callback`: The reference to the function to remove.
*   **Returns**:
    *   `void`
*   **Behavior**:
    *   Removes the specific callback from the subscription list.
...
*   **Params**:
    ...
    *   `type`: The event type (pressed/held).
*   **Returns**:
    *   `void`
*   **Behavior**:
    *   Executes all callbacks associated with the key/type pair.
    *   Mostly used internally by the input system but can simulate inputs.
*   **Example**:

[source,typescript]
----
control.notify(ControlKey.EXIT, ControlEventType.PRESSED);
----
