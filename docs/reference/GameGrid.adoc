...
`Cell[][]` - The matrix representing the grid state.
...
|`callback` |`(cell: Cell) => void` |Function to execute for each cell.
|===
...
|===
|Name |Type |Description
|`coordinate` |`Coordinate` |The `{x, y}` point to validate.
|===
...
|===
|Name |Type |Description
|`coordinate` |`Coordinate` |The `{x, y}` position to query.
|===

**Returns**

`Cell | null` - The cell object, or `null` if the coordinate is out of bounds.
...
Updates only the `value` property of the cell. If the coordinate is invalid, the operation is ignored.
...
|Name |Type |Description
|`coordinate` |`Coordinate` |Target position.
|`color` |`Color` |Enum value representing the new color.
|===
...
|===
|Name |Type |Description
|`coordinates` |`Coordinate[]` |Array of `{x, y}` points representing a shape or piece.
|===
...
|===
|Name |Type |Description
|`start` |`Coordinate` |Top-left corner of the rectangle.
|`end` |`Coordinate` |Bottom-right corner of the rectangle.
|`value` |`number` |Value to apply to all cells in the area.
|`color` |`Color` |Color to apply to all cells in the area.
|===
