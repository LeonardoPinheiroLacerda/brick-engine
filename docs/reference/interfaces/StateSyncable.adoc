...
Implemented by `xref:../modules/GameSound.adoc[GameSound]`. During the start-up sequence, the engine calls `syncState()` on these modules to ensure they have the latest references to `muted` flags and other state data.
