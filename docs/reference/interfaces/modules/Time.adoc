= Time Reference
:description: Interface for managing the game's logical clock and update frequency.
:keywords: time, clock, tick, interval, game-loop
:toc: left
:toclevels: 3
:sectnums:
:sectlinks:
:icons: font
:source-highlighter: highlight.js
:reproducible:
:experimental:
:idprefix:
:idseparator: -

Interface for managing the game's logical clock and update frequency.

== Properties

[cols="1,1,3"]
|===
|Name |Type |Description
|`tickInterval` |`number` |The duration in milliseconds between logic updates.
|`totalTicks` |`number` |The total number of logic ticks that have occurred since the game started.
|`elapsedTime` |`number` |The total time in milliseconds elapsed since the game started.
|===

== Methods

=== `update(deltaTime: number): void`

Adds elapsed time to the internal tick accumulator. Called every frame.

**Parameters**

[cols="1,1,3"]
|===
|Name |Type |Description
|`deltaTime` |`number` |Time in milliseconds since last frame.
|===


'''

=== `shouldTick(): boolean`

Returns `true` if the tick accumulator has exceeded the `tickInterval`, indicating a logic update is due.

**Returns**

`boolean` - `true` if a tick should occur.


'''

=== `reset(): void`

Clears the tick accumulator and resets global time counters. Useful during game restarts.


'''

=== `isTickEvery(interval: number): boolean`

Returns `true` if the `totalTicks` is a multiple of the specified `interval`. Useful for scheduling recurring game events.

**Parameters**

[cols="1,1,3"]
|===
|Name |Type |Description
|`interval` |`number` |The tick frequency to check.
|===

**Returns**

`boolean` - `true` if the current tick is a multiple of the interval (and greater than 0).


'''

=== `incrementTickInterval(amount: number): void`

Slows down the game by increasing the time between ticks.

**Parameters**

[cols="1,1,3"]
|===
|Name |Type |Description
|`amount` |`number` |Milliseconds to add to the interval.
|===


'''

=== `decrementTickInterval(amount: number): void`

Speeds up the game by decreasing the time between ticks.

**Parameters**

[cols="1,1,3"]
|===
|Name |Type |Description
|`amount` |`number` |Milliseconds to subtract from the interval.
|===


'''

=== `setTickInterval(interval: number): void`

Sets the tick interval exactly to the specified value.

**Parameters**

[cols="1,1,3"]
|===
|Name |Type |Description
|`interval` |`number` |The new interval between logic updates (in milliseconds).
|===


'''

=== `setMinTickInterval(interval: number): void`

Sets the minimum limit for the tick interval, preventing the game from speeding up beyond this point.

**Parameters**

[cols="1,1,3"]
|===
|Name |Type |Description
|`interval` |`number` |The minimum interval allowed (in milliseconds).
|===


'''

=== `captureInitialState(): void`

Captures the current `tickInterval` as the initial state. This should be called after a game has performed its specific initialization to ensure that future resets restore the correct starting speed.

== General Usage

Implemented by `xref:../../modules/GameTime.adoc[GameTime]`. This module ensures that game logic (like falling pieces) runs at a consistent speed regardless of the render frame rate.
