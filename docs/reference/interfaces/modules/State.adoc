...
Returns `true` if the trackpad input method is enabled.

**Returns**

`boolean` - `true` if the trackpad is active.
...
Manually enables or disables the trackpad input method.
...
|===
|Name |Type |Description
|`value` |`boolean` |`true` to enable trackpad, `false` for classic buttons.
|===
...
Toggles the trackpad input method.
