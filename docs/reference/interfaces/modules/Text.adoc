...
=== `pulsingTextOnHud(text: string, coordinate: xref:../../types/Coordinate.adoc[Coordinate], elapsedTime: number, pulseSpeedMs?: number): void`
...
|`text` |`string` |The string content to display.
|`coordinate` |`xref:../../types/Coordinate.adoc[Coordinate]` |Normalized position `{x, y}`.
|`elapsedTime` |`number` |Total elapsed time in milliseconds from `time.elapsedTime`.
...
=== `pulsingTextOnDisplay(text: string, coordinate: xref:../../types/Coordinate.adoc[Coordinate], elapsedTime: number, pulseSpeedMs?: number): void`
