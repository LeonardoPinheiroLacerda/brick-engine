...
As defined in the Architectural Patterns, `GameState` is responsible for all `LocalStorage` logic. Other modules (like `GameSound`) must sync their internal state with this module via the `StateSyncable` interface.
...
|===
|Name |Type |Description
|`property` |`StateProperty` |The specific property to monitor.
...
|===
|Name |Type |Description
|`property` |`StateProperty` |The property being monitored.
